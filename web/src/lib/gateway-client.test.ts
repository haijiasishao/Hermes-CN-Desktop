...
describe("GatewayClient", () => {
  ...
  describe("connection-failover event (Android Remote)", () => {
    function setupAndroidFailoverWindow(failoverCaptured: { handler: ((payload: unknown) => void) | null }) {
      vi.stubGlobal("window", {
        location: { href: "http://localhost:9545/tasks/s1", protocol: "http:" },
        __HERMES_SESSION_TOKEN__: "token",
        __HERMES_RUNTIME__: { androidRemoteOnly: true },
        hermesDesktop: {
          onConnectionFailover: (cb: (payload: unknown) => void) => {
            failoverCaptured.handler = cb;
            return () => { failoverCaptured.handler = null; };
          },
        },
        addEventListener: () => {},
        removeEventListener: () => {},
    ...
    it("Android Remote: failover event tears down OPEN socket and reconnects immediately", async () => {
      const failoverCaptured: { handler: ((payload: unknown) => void) | null } = { handler: null };
      setupAndroidFailoverWindow(failoverCaptured);
      ...
      // Simulate Rust emitting connection-failover after primary endpoint died.
      failoverCaptured.handler?.({
        fromUrl: "http://192.168.1.10:9119",
        toUrl: "http://external:9119",
        activeRemote: "backup",
        failoverActive: true,
      });

      // Old socket must be torn down.
      expect(MockWebSocket.instances[0].readyState).toBe(MockWebSocket.CLOSED);
      // A new socket must have been opened for the reconnect.
      expect(MockWebSocket.instances).toHaveLength(2);
    ...
    it("desktop (non-Android): failover event does NOT tear down the socket", async () => {
      const failoverCaptured: { handler: ((payload: unknown) => void) | null } = { handler: null };
      vi.stubGlobal("window", {
        location: { href: "http://localhost:9545/tasks/s1", protocol: "http:" },
        __HERMES_SESSION_TOKEN__: "token",
        __HERMES_RUNTIME__: { androidRemoteOnly: false },
        hermesDesktop: {
          onConnectionFailover: (cb: (payload: unknown) => void) => {
            failoverCaptured.handler = cb;
            return () => { failoverCaptured.handler = null; };
      ...
      // Desktop must NOT be affected — the socket stays OPEN.
      expect(MockWebSocket.instances).toHaveLength(1);
      expect(MockWebSocket.instances[0].readyState).toBe(MockWebSocket.OPEN);
      expect(client.state).toBe("open");
    ...
    it("Android Remote: failover event during connecting state does NOT force reconnect", async () => {
      const failoverCaptured: { handler: ((payload: unknown) => void) | null } = { handler: null };
      setupAndroidFailoverWindow(failoverCaptured);
      ...
      // Start connecting but do NOT open the socket yet — readyState is CONNECTING.
      const _connecting = client.connect().catch(() => {});
      expect(MockWebSocket.instances).toHaveLength(1);
      ...
      // No extra socket should be spawned — the in-progress connect is left alone.
      expect(MockWebSocket.instances).toHaveLength(1);

      client.close();
    });

    it("Android Remote: failover listener is cleaned up on close", async () => {
      const failoverCaptured: { handler: ((payload: unknown) => void) | null } = { handler: null };
      ...
      // The unlisten function must have been called during cleanup.
      expect(unlistenSpy).toHaveBeenCalled();
      expect(failoverCaptured.handler).toBeNull();
    });
  });
});
